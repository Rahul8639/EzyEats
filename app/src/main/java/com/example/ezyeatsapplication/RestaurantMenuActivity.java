...
import com.example.ezyeatsapplication.adapters.MenuListAdapter;
import com.example.ezyeatsapplication.model.Menu;
...
public class RestaurantMenuActivity extends AppCompatActivity implements MenuListAdapter.MenuListClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.fragment_activity_restaurant_menu);
        ...
        if (actionBar != null) {
            ...
            actionBar.setDisplayHomeAsUpEnabled(true); // Enable back navigation
        }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                finish();  // Close activity when back button on action bar is pressed
                return true;
            default:
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 1000 && resultCode == Activity.RESULT_OK) {
            finish();  // Finish activity after returning from PlaceYourOrderActivity
        }
    }
}
